...
Goal_H = 7
Goal_W = 6
Image_size = UNIT * MAZE_H
Title_size = RECT_SIZE
Scale = 1.2
...
class Maze(tk.Tk, object):
    def __init__(self,
        ...
        self.video = cv2.VideoWriter(video_name, fourcc, 15, (int(Image_size/Scale), int((Image_size+Title_size)/Scale)))
        self.capture_images = []
        self.title_str = ""
    ...
    def reset(self, net, episode=0, key_episode=30):
        if episode > 0:
            ...
            title_image = np.ones((Title_size, Image_size, 3)) * 255.0
            ...
            img = cv2.resize(img, (Image_size, Image_size + Title_size))
    ...
    def _built_maze(self):
        ...
        # create paradise
        oval_center = self.origin + np.array([UNIT * Goal_H, UNIT * Goal_W])
    ...
    def draw_all_q_value(self, net, episode=0, key_episode=30):
        ...
        for i in range(maze_h):
            for j in range(maze_w):
                center_pos = np.array([UNIT * i + UNIT/2.0, UNIT * j + UNIT/2.0])
                oval_coords = self.canvas.coords(self.oval)
                ...
                if arrow_pos:
                    ...
                    q_value_pos_list.append([center_pos[0] + txt_bias,
                                             center_pos[1]])
                    next_state = [center_pos[0]-UNIT/2+2,
                                  center_pos[1]-UNIT/2+8,
                                  center_pos[0]+UNIT/2,
                                  center_pos[1]+UNIT/2,
                                  ]
    ...
    def step(self, action, net, step=0, episode=0, key_episode=30):
        ...
        if step % 20 == 0:
            ...
            img = np.concatenate((title_image, img), axis=0)
            # print("img:", img.shape)
            img = cv2.resize(img, (int(Image_size/Scale), int((Image_size+Title_size)/Scale)))
            img = np.uint8(img)
            # print("img:", img.shape)
            self.capture_images.append(img)
